...
const DEFAULT_RUN_DIR = resolve(HERE, "..", "research-reframer-skills-codex", "examples", "physmaster-three-gate");
...
async function sendToCurrentThread(ids, gateNumber) {
  ...
  if (THREAD_ID === "antigravity") {
    await writeGateStatus({
      ...
      message: `已记录 Gate ${gateNumber} 选择：${ids.join(", ")}。等待 Antigravity 执行下一步。`,
    });
    return {
      ok: true,
      mode: "antigravity",
      gate: gateNumber,
      completed: false,
      note: "Selection recorded locally. Waiting for Antigravity agent process to poll and execute."
    };
  }
  await writeGateStatus({
    gate: gateNumber,
...
async function loadCompleteState() {
  return {
    gate: 4,
    heading: "Research Reframer 工作坊已完成",
    task: "流程已完成",
    runDir: RUN_DIR,
    optionsPath: `${OUTPUTS_DIR}\\06_idea_cards.json`,
    recommendedIds: [],
    selectionGuidance: "恭喜！工作坊研讨流程已全部顺利完成。生成的科研卡片与报告已存入 outputs 目录，可直接预览。",
    options: [],
  };
}
...
async function loadGate1Options() {
  ...
  return {
    gate: 1,
    heading: "Gate 1：定位系统杠杆点",
    task: "定位系统杠杆点",
    runDir: RUN_DIR,
    optionsPath: path,
    recommendedIds,
    selectionGuidance: "选择后，系统仅围绕这些核心杠杆点进行横向发散。本阶段重在定位破局点，并非选定最终科研方向。",
...
async function loadGate2Options() {
  ...
  const options = (doc.lateral_schemes || []).map((scheme) => ({
    ...
    summaryRows: rows(
      ...
      { label: "方案", text: scheme.scheme, max: 110 },
      { label: "创新亮点", text: scheme.why_interesting, max: 110 },
  ...
  return {
    gate: 2,
    heading: "Gate 2：筛选创新发散思路",
    task: "筛选创新发散思路",
    runDir: RUN_DIR,
    optionsPath: path,
    recommendedIds,
    selectionGuidance: "在此评估值得进入严苛审计的候选构想。优先选择具备清晰科学直觉与明确失效风险的思路。",
    options: options.map((o) => ({ ...o, recommended: recommendedIds.includes(o.id) })),
  };
}
...
async function loadGate3Options() {
  ...
  const options = audits
    ...
    .map((audit) => ({
      ...
      detailRows: rows(
        { label: "对抗评审意见", text: Array.isArray(audit.reasons) ? audit.reasons.join(" ") : audit.reasons, max: 220 },
        { label: "核心因果机制", text: audit.causal_mechanism, max: 180 },
        { label: "评审综合分 (Overall Score)", text: audit.audit_score ? `overall ${audit.audit_score.overall}` : "", max: 80 }
      ),
    }));
  const recommendedIds = recommendedFor(3, options.map((o) => o.id));
  return {
    gate: 3,
    heading: "Gate 3：筛选高可行性科研构想",
    task: "筛选高可行性科研构想",
    runDir: RUN_DIR,
    optionsPath: path,
    recommendedIds,
    rejectedIds,
    selectionGuidance: "本阶段评估通过 AI 对抗审查后的改良构想、最小实验设计及边界风险。已被双重否决的方案已自动过滤。",
    options: options.map((o) => ({ ...o, recommended: recommendedIds.includes(o.id) })),
  };
}
...
function stateLabel(state) {
  const labels = {
    idle: "等待决策选择",
    complete: "已完成",
    recording_selection: "正在记录决策",
    connecting: "正在连接服务...",
    reading_thread: "正在读取工作流状态...",
    resuming_thread: "正在同步探索线程...",
    starting_turn: "准备启动 AI 研讨...",
    agent_running: "AI 协同研讨中...",
    artifact_ready: "工件生成完毕",
    timed_out: "等待超时",
    failed: "失败",
    waiting_for_agent: "等待 AI 处理",
  };
  return labels[state] || state || "等待决策选择";
}
...
function renderChangeLog(card) {
  const labels = {
    source_diagnosis: "源缺陷诊断",
    selected_leverage_point: "所选杠杆点",
    structural_change: "结构层改变",
    new_mechanism: "新增关键机制",
    preserved_from_original: "保留的核心价值",
    removed_or_weakened: "被削弱或剔除的声称",
    real_increment_over_original: "真实增量贡献",
    remaining_fragility: "剩余脆弱性/不确定度",
  };
...
function renderTechnicalDetails(card) {
  return `
    <details class="technicalDetails">
      <summary>展开技术实现细节、数据追溯与原始字段</summary>
      <div class="technicalBody">
        ${renderIdeaTrace(card)}
        <div class="ideaGrid">
          ${renderIdeaField("原前提假说", card.original_problem)}
          ${renderIdeaField("重塑后的问题", card.reframed_problem)}
        ...
        <section class="experimentBlock">
          ...
          ${renderIdeaList("评估指标", card.evaluation_metrics)}
          ${renderIdeaField("预期观测现象", card.expected_observation)}
          ${renderIdeaField("实验失败判据", card.failure_case)}
        </section>
        <div class="ideaGrid">
          ${renderIdeaList("文献检索词 (Related Work)", card.related_work_queries)}
          ${renderIdeaList("后续推进计划", card.next_steps)}
        </div>
...
function renderIdeaCard(card, index) {
  ...
  return `
    <article class="ideaCard">
      <div class="ideaHead">
        <div>
          <span class="ideaKicker">${escapeHtml(cardId)} · 概念概要${card.status ? ` · ${escapeHtml(card.status)}` : ""}</span>
          <h2>${escapeHtml(reader.headline)}</h2>
        </div>
      </div>
      ${reader.summary ? `<p class="ideaLead">${escapeHtml(reader.summary)}</p>` : ""}
      <div class="readerGrid">
        ${renderReaderBox("默认共识 (传统前提)", reader.original_default)}
        ${renderReaderBox("重定义的问题 (新视角)", reader.new_question)}
        ${renderReaderBox("最小可行验证 (实验设计)", reader.minimal_test)}
        ${renderReaderBox("核心风险与局限性 (边界条件)", reader.main_risk)}
      </div>
      ${renderReaderBox("学术价值与科学意义", reader.why_it_matters)}
  ...
  return `
    <section class="finalDeck">
      <div class="finalHeader">
        ...
        <h2>最终科研卡片 (Research Idea Cards)</h2>
        <p>你在 Gate 3 选中的方案已被成功编译为科研卡片；以下为直观的概念概要，供阅读与立项评估。</p>
      </div>
      ${
        ideaCards ||
...
async function html() {
  ...
  const gatePanel =
    ...
    <div class="panel">
      ...
      <div class="row">
        <button onclick="sendSelected()">确认选择并推进工作流</button><button class="secondary" onclick="selectRecommended()">采用推荐配置</button><button class="secondary" onclick="clearSelected()">重置所选</button>
      </div>
    </div>`;
...
<body>
  <main>
    ...
    <div class="panel progressPanel">
      ...
      <div class="meterMeta">
        <span id="meterLabel">工作流全局进度（根据工件状态计算）</span>
      ...
      <div class="statusFacts">
        <div class="fact"><span>当前决策关卡</span><strong id="factGate">Gate ${gate.gate}</strong></div>
        <div class="fact"><span>当前生成目标</span><strong id="factArtifact">${escapeHtml(progress.expected_artifact_name || "等待决策选择")}</strong></div>
        <div class="fact"><span>已选决策 ID</span><strong id="factIds">${escapeHtml((progress.selected_ids || []).join(", ") || "尚未提交")}</strong></div>
        <div class="fact"><span>最近系统同步时间</span><strong id="factHeartbeat">${escapeHtml(progress.last_heartbeat || progress.updated_at || "尚未开始")}</strong></div>
      </div>
      <div class="stageRail" id="stageRail">${stageRows}</div>
    </div>

    <div class="panel">
      <strong>系统状态维护</strong>
      <p>同步读取当前的探索线程与关卡状态，不消耗计算配额。</p>
      <div class="row">
        <button class="secondary" onclick="probe()">诊断探索线程</button>
        <button class="secondary" onclick="location.reload()">重新加载页面</button>
      </div>
    </div>
