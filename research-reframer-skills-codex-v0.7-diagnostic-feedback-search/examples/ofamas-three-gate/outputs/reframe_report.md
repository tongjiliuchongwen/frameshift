# Reframe report — OFA-MAS (v0.7 diagnostic three-gate demo)

**Input:** OFA-MAS 一对多 MAS 拓扑设计 (arXiv:2601.12996v1) paper-gap. **Contract:** v0.7 / schema_version 2.1.

## What the three gates produced
0. **Diagnosis:** named the primary bottleneck: the original frame attributes gains to learned
   topology without cleanly separating topology from node content, routing, and synthetic-data
   quality.
...
## What v0.7 adds
- `00_diagnosis.json` sets the run's target before system mapping begins.
- System-map relations are now auditable claims with relation type, evidence source, mechanism, and
  wrong-edge impact.
- `04_vertical_audits.json` records pseudo-innovation status and failure types instead of burying
  "renamed known method" in prose.
- `06_idea_cards.json` includes `change_log`, so each card states what structural change was made and
  what real increment it adds over the input.
